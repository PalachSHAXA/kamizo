...
import { getTenantId, setTenantForRequest, getTenantForRequest, getTenantSlug, setCurrentTenant, getCurrentTenant, clearFeatureCache } from './middleware/tenant';
...
route('POST', '/api/auth/login', async (request, env) => {
  ...
  // On subdomain: find users belonging to that tenant OR super_admin users
  // On main domain (app.kamizo.uz): find ANY user across all tenants (unified login)
  const tenantId = getTenantId(request);
  let userWithHash: any = null;
  if (tenantId) {
    // Subdomain login: filter by tenant + allow super_admin
    userWithHash = await env.DB.prepare(
      `SELECT id, login, phone, name, role, specialization, address, apartment, building_id, branch, building, entrance, floor, total_area, password_hash, password_changed_at, contract_signed_at, account_type, tenant_id FROM users WHERE login = ? AND (tenant_id = ? OR (role = 'super_admin' AND (tenant_id IS NULL OR tenant_id = ''))) AND is_active = 1`
    ).bind(login.trim(), tenantId).first() as any;
  } else {
    // Main domain login: search across ALL tenants (unified entry point for mobile app)
    // Same login can exist in multiple tenants — try each until password matches
    const { results: candidates } = await env.DB.prepare(
      `SELECT id, login, phone, name, role, specialization, address, apartment, building_id, branch, building, entrance, floor, total_area, password_hash, password_changed_at, contract_signed_at, account_type, tenant_id FROM users WHERE login = ? AND is_active = 1 ORDER BY CASE WHEN role = 'super_admin' THEN 0 ELSE 1 END LIMIT 10`
    ).bind(login.trim()).all() as { results: any[] };
  ...
  // Verify password — already verified for main domain (multi-tenant search above)
  // For subdomain login, verify now
  if (tenantId) {
...
// Toggle feature for own tenant (managers/directors only)
route('PATCH', '/api/tenant/features', async (request, env) => {
  const user = await getUser(request, env);
  if (!user || !isManagement(user)) return error('Access denied', 403);
  
  const tenantId = getTenantId(request);
  if (!tenantId) return error('Tenant required', 400);

  const body = await request.json() as { feature: string; enabled: boolean };
  if (!body.feature) return error('Feature name required');

  const tenant = await env.DB.prepare('SELECT features FROM tenants WHERE id = ?').bind(tenantId).first() as any;
  if (!tenant) return error('Tenant not found', 404);

  let features: string[] = [];
  try { features = tenant.features ? JSON.parse(tenant.features) : []; } catch { features = []; }

  if (body.enabled && !features.includes(body.feature)) {
    features.push(body.feature);
  } else if (!body.enabled) {
    features = features.filter((f: string) => f !== body.feature);
  }

  await env.DB.prepare('UPDATE tenants SET features = ? WHERE id = ?')
    .bind(JSON.stringify(features), tenantId).run();
...
route('GET', '/api/tenant/config', async (request, env) => {
  ...
  // On main domain (app.kamizo.uz), resolve tenant from logged-in user
  if (!tenant) {
    try {
      const user = await getUser(request, env);
      if (user && user.tenant_id) {
        tenant = await env.DB.prepare('SELECT * FROM tenants WHERE id = ?').bind(user.tenant_id).first() as any;
      }
    } catch {}
  }
...
route('POST', '/api/super-admin/reset-password', async (request, env) => {
  const user = await getUser(request, env);
  if (!isSuperAdmin(user)) return error('Access denied', 403);

  const body = await request.json() as any;
  const { user_id, password } = body;
  if (!user_id || !password) return error('user_id and password are required');

  const targetUser = await env.DB.prepare('SELECT id, login, name, role FROM users WHERE id = ?').bind(user_id).first() as any;
...
// POST /api/_emergency-reset - one-time emergency password reset (remove after use!)
route('POST', '/api/_emergency-reset', async (request, env) => {
  ...
  await env.DB.prepare('UPDATE users SET password_hash = ? WHERE id = ?').bind(hashedPwd, body.user_id).run();
  return json({ success: true });
});

// POST /api/super-admin/fix-passwords - hash all plaintext passwords that have no hash
route('POST', '/api/super-admin/fix-passwords', async (request, env) => {
  const user = await getUser(request, env);
  if (!isSuperAdmin(user)) return error('Access denied', 403);
