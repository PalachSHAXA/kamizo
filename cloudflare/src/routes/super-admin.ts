...
import { DEFAULT_TENANT_FEATURES, normalizeFeatures } from '../lib/features';
...
// ──────────────────────────────────────────────────────────────────
// D1 dual-write mirror для tenants вынесен в lib/tenantMirror.ts —
// им же пользуется PATCH /api/tenant/features (routes/misc/health.ts).
...
route('POST', '/api/tenants', async (request, env) => {
  ...
  const id = generateId();
  // Нормализуем то, что прислала форма, и падаем на канонический дефолт.
  // Раньше дефолт содержал легаси-ключ "votes", которого не знает ни один
  // гейт, — у нового тенанта раздел «Собрания» сразу был недоступен.
  const requestedFeatures = normalizeFeatures(body.features);
  const features = JSON.stringify(
    requestedFeatures.length > 0 ? requestedFeatures : DEFAULT_TENANT_FEATURES,
  );
...
route('PATCH', '/api/tenants/:id', async (request, env, params) => {
  ...
  if (body.features !== undefined) {
    ...
    values.push(JSON.stringify(normalizeFeatures(body.features)));
  }
