...
route('POST', '/api/vehicles', async (request, env) => {
  ...
  await env.DB.prepare(`
    INSERT INTO vehicles (id, user_id, plate_number, brand, model, color, year, vehicle_type, owner_type, company_name, parking_spot, notes, is_primary, tenant_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id, user.id, normalizedPlate,
...
route('PATCH', '/api/vehicles/:id', async (request, env, params) => {
  ...
  await env.DB.prepare(`UPDATE vehicles SET ${updates.join(', ')} WHERE id = ? AND user_id = ? ${tenantId ? 'AND tenant_id = ?' : ''}`).bind(...values).run();
...
route('DELETE', '/api/vehicles/:id', async (request, env, params) => {
  ...
  const tenantId = getTenantId(request);
  await env.DB.prepare(`DELETE FROM vehicles WHERE id = ? AND user_id = ? ${tenantId ? 'AND tenant_id = ?' : ''}`).bind(params.id, user.id, ...(tenantId ? [tenantId] : [])).run();
...
route('GET', '/api/vehicles/all', async (request, env) => {
  ...
  const { total } = await env.DB.prepare(`
    SELECT COUNT(*) as total FROM vehicles v JOIN users u ON v.user_id = u.id ${whereClause}
