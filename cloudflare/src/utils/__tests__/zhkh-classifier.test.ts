...
// ── Короткие корни ищутся с начала слова ─────────────────────────────
//
// Самая живучая ошибка этого классификатора. normalize() удаляет
// пробелы, и короткий корень начинал совпадать в середине чужого слова.
// Три раза это лечили удалением корня ('том', 'бор', 'урна') — то есть
// симптома, а не причины. Теперь корни до четырёх букв проверяются по
// тексту с сохранёнными границами слов.
describe('короткие корни и границы слов', () => {
  it.each([
    ['Позвоните в аварийку, там не берут трубку', 'вон в «позВОНите»'],
    ['Провода висят, но свет есть', 'вода в «проВОДА»'],
    ['Молотком разбили стекло', 'ток в «молоТОК»'],
    ['Показали новый двор, не узнать', 'зали в «покаЗАЛИ»'],
    ['Табак курить в подъезде нельзя', 'бак в «таБАК»'],
    ['Культурная программа во дворе не состоялась', 'урна в «кульТУРНАя»'],
    ['Растения во дворе засохли, никто не поливает', 'стен в «раСТЕНия»'],
    ['Встретимся на пятом этаже', 'том в «пяТОМ»'],
    ['Забор во дворе покрасили', 'бор в «заБОР»'],
  ])('не срабатывает: %s (%s)', (text) => {
    expect(fires(text)).toBe(false);
  });

  // Обратная сторона: с начала слова те же корни обязаны находиться.
  it.each([
    'Нет воды со вчера',
    'Холодная вода еле идёт',
    'Свет вырубило в подъезде',
    'Урна переполнена у входа',
    'Том оқяпти, охирги қаватда',
  ])('срабатывает: %s', (text) => {
    expect(fires(text)).toBe(true);
  });

  // Агглютинация не пострадала: узбекский корень стоит в начале слова,
  // а окончания вроде 'yapti' длиннее порога и ищутся по склейке.
  it.each([
    'Podezdda suv oqyapti',
    'Lift ishlamayapti',
    'Kran sindi',
    'Suvni tozalash kerak emas, kran sindi',
  ])('узбекская агглютинация: %s', (text) => {
    expect(fires(text)).toBe(true);
  });

  // Пропущенный пробел перед коротким корнем — известная цена решения.
  // Тест фиксирует её явно, чтобы это не выглядело случайностью.
  it('пропущенный пробел перед коротким корнем — известная потеря', () => {
    expect(fires('лифт неработает')).toBe(true);      // длинный корень цел
    expect(fires('нетгорячейводы')).toBe(false);      // короткий — потерян
  });
});

// ── Уборка территории ────────────────────────────────────────────────
//
// Категория добавлена последней и почти целиком пересекается с мусором,
// поэтому проверяется в первую очередь граница между ними.
describe('уборка', () => {
  it.each([
    'Уборщицы не было всю неделю, в подъезде грязь',
    'Снег не чистят у второго подъезда',
    'Гололёд у входа, никто не посыпал',
    'Плохо убирают двор, листья не метут',
    'В подъезде грязно, не подметают',
    'Сосульки нависают над входом',
    'Podezdni tozalashmayapti, iflos',
    'Farrosh kelmadi, juda iflos',
  ])('ловит: %s', (text) => {
    expect(classifyZhkhMessage(text)?.category).toBe('cleaning');
  });

  // Названный мусор — это вывоз мусора, а не уборка. Категория с
  // большим числом совпадений побеждает, при ничьей — объявленная
  // раньше, поэтому cleaning стоит в коде после garbage.
  it.each([
    'Мусор не вывозят третий день, контейнеры переполнены',
    'Мусоропровод забился на 5 этаже, воняет на весь подъезд',
    'Контейнер переполнен, всё вокруг завалено',
  ])('остаётся мусором: %s', (text) => {
    expect(classifyZhkhMessage(text)?.category).toBe('garbage');
  });

  it.each([
    'Завтра во дворе субботник, приходите с мётлами',
    'Снег на улице сегодня красивый',
    'Спасибо, двор убрали',
  ])('молчит: %s', (text) => {
    expect(fires(text)).toBe(false);
  });
