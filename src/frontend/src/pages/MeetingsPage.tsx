      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl md:text-2xl font-bold text-gray-900 truncate">{t('meetings.title')}</h1>
            <p className="text-xs text-gray-500 mt-0.5 truncate">{t('meetings.subtitle')}</p>
          </div>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-br from-[#E8621A] to-[#F59E0B] text-white rounded-xl shadow-sm hover:opacity-90 transition-opacity font-medium text-sm shrink-0"
        >
          <Plus className="w-5 h-5" />
          <span className="hidden sm:inline">{t('meetings.create')}</span>
        </button>
      </div>
