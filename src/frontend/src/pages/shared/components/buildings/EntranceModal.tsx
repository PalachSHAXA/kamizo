...
export function EntranceModal({ onClose, onSave, existingEntrances, language }: EntranceModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-[110]" onClick={onClose}>
      <div className="bg-white/90 backdrop-blur-xl rounded-t-2xl sm:rounded-2xl w-full max-w-md max-h-[90dvh] overflow-y-auto p-6 border border-white/60 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={e => { e.preventDefault(); onSave(form); }} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wide mb-1.5">{t('Номер подъезда', 'Podyezd raqami')} *</label>
            <input type="number" inputMode="numeric" pattern="[0-9]*" value={form.number} onChange={e => setForm({ ...form, number: parseInt(e.target.value) || 1 })}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" inputMode="numeric" pattern="[0-9]*" value={form.floors_from} onChange={e => setForm({ ...form, floors_from: parseInt(e.target.value) || 1 })}
            ...
            <div>
              ...
              <input type="number" inputMode="numeric" pattern="[0-9]*" value={form.floors_to} onChange={e => setForm({ ...form, floors_to: parseInt(e.target.value) || 1 })}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-orange-400 outline-none" min="1" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" inputMode="numeric" pattern="[0-9]*" value={form.apartments_from} onChange={e => setForm({ ...form, apartments_from: parseInt(e.target.value) || 1 })}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-orange-400 outline-none" min="1" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wide mb-1.5">{t('Квартира по', 'Xonadongacha')}</label>
              <input type="number" inputMode="numeric" pattern="[0-9]*" value={form.apartments_to} onChange={e => setForm({ ...form, apartments_to: parseInt(e.target.value) || 1 })}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-orange-400 outline-none" min="1" />
            </div>
          </div>
