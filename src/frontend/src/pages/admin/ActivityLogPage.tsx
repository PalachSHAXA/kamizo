      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl md:text-2xl font-bold text-gray-900 truncate">{language === 'ru' ? 'Журнал действий' : 'Harakatlar jurnali'}</h1>
            <p className="text-xs text-gray-500 mt-0.5 truncate">{language === 'ru' ? 'История всех операций в системе' : 'Tizimdagi barcha operatsiyalar tarixi'}</p>
          </div>
        </div>
        <button onClick={handleExportActivityLog} className="btn-secondary flex items-center gap-2 shrink-0">
          <Download className="w-4 h-4" />
          <span className="hidden sm:inline">{language === 'ru' ? 'Экспорт CSV' : 'CSV eksport'}</span>
        </button>
      </div>
