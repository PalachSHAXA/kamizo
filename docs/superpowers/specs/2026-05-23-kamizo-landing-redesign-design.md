...
### 5.10 ClientsSection — **реальный My Helper**

Карточка с реальными данными:
- **Логотип:** УК «My Helper»
- **Тип:** Управляющая компания
- **Жители:** **5 000+** (подтверждено пользователем)
- **Сайт:** `myhelper.uz`
- **Приложение:** `app.myhelper.uz`
- **Рейтинг:** **запросить настоящий, иначе убрать** (Kimi показывал 4.4, но это плейсхолдер)
- **Отзыв:** **запросить настоящий, иначе убрать секцию отзыва** (Kimi показывал «TBA»)
...
CTA: «Получать обновления» (форма email → новый эндпоинт или Mailchimp).
...
1. **Точный рейтинг и отзыв My Helper** — что писать? Если нет — убираем оба поля и просто показываем «5 000+ жителей · myhelper.uz».
2. **Email-подписка в ComingSoon** — куда отправлять (Mailchimp / новый API / `mailto:`)?
3. **EN-переводы** — делаем сейчас, или сначала RU+UZ, EN добавляем post-deploy?

Эти три можно ответить после approve'а спека.
